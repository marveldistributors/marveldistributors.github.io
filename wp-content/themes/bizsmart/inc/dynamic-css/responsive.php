 ...
 */
function bizsmart_responsive_device_css(){
	foreach( array( 'md' => 'desktop', 'sm' => 'tablet', 'xs' => 'mobile' ) as $size => $device ){

		BizSmart_Css::add_styles( array(
			#Typography
			array(
			...
			array(
				'selector'  => BizSmart_Helper::with_prefix_selector( '%s-post .post-title a' ),
				'props'		=> array(
					'font-size' => 'post-title-size-' . $device,
				),
			),
			array(
				'selector' => BizSmart_Helper::with_prefix_selector( '%s-main-menu > ul > li > a' ),
				'props'	   => array(
			...
			array(
				'selector' => BizSmart_Helper::with_prefix_selector( '%s-inner-banner-wrapper' ),
				'props' => array(
...
add_action( 'init', 'bizsmart_responsive_device_css' );
