...
!function (t, n) {
  n.sectionConstructor["bizsmart-anchor"] = n.Section.extend({
...
wp.customize.controlConstructor['bizsmart-buttonset'] = wp.customize.Control.extend({
  ...
  jQuery(document).ready(function ($) {
    ...
    $('.alpha-color-control').each(function () {
      ...
      colorPickerOptions = {
        ...
        palettes: bizsmartColorPalette.colorPalettes // Use the passed in palette.
      ...
      $('<div class="alpha-bizsmart-color-picker-container">' + '<div class="min-click-zone click-zone"></div>' + '<div class="max-click-zone click-zone"></div>' + '<div class="alpha-slider"></div>' + '<div class="transparency"></div>' + '</div>').appendTo($container.find('.wp-picker-holder'));
...
wp.customize.controlConstructor['bizsmart-dimensions'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(document).ready(function ($) {
  ...
  $('.linked').on('click', function () {
    ...
    $this.parent('.link-bizsmart-dimensions').removeClass('unlinked');
  }); // Unlinked button
  ...
  $('.unlinked').on('click', function () {
    ...
    $this.parent('.link-bizsmart-dimensions').addClass('unlinked');
...
wp.customize.controlConstructor['bizsmart-icon'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(window).load(function () {
  ...
  jQuery('.customize-control-bizsmart-radio-image .buttonset').buttonset(); // Handles setting the new value in the customizer.

  jQuery('.customize-control-bizsmart-radio-image input:radio').change(function () {
...
wp.customize.controlConstructor['bizsmart-range'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
    ...
    var control = this,
        ...
        input_default,
        changeAction,
        bizsmart_range_input_number_timeout; // Update the text value
    ...
    function autocorrect_range_input_number(input_number, timeout) {
      ...
      clearTimeout(bizsmart_range_input_number_timeout);
      bizsmart_range_input_number_timeout = setTimeout(function () {
        if (isNaN(value)) {
...
wp.customize.controlConstructor['bizsmart-slider'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
(function ($, api) {
  api.controlConstructor['bizsmart-toggle'] = api.Control.extend({
    ready: function ready() {
      var control = this;
