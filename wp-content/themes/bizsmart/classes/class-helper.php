 ...
 */
if( !class_exists( 'BizSmart_Helper' ) ):
	class BizSmart_Helper{
		...
		public static function skip_content(){
			...
			<a class="skip-link screen-reader-text" href="#content">
				<?php esc_html_e( 'Skip to content', 'bizsmart' ); ?>
			</a>
			<?php
		}
		...
		public static function excerpt_more( $text = false ){
			...
			if( '' != bizsmart_get( 'read-more-text' ) ){				
				$more = sprintf( '<a href="%1$s" class="bizsmart-read-more">%2$s<i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>',
		...
		*/
		...
		public static function get_comment_number(){
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				echo '<span class="bizsmart-comments">';
				comments_popup_link(
					'<i class="fa fa-comment"></i> '.esc_html__( 'Leave a comment', 'bizsmart' ),		
					'<i class="fa fa-comment"></i> '.esc_html__( '1 response', 'bizsmart' ),
					'<i class="fa fa-comments"></i> % '. esc_html__( 'responses' , 'bizsmart' )
				);
				echo '</span>';
			}
		}		
		...
		*/
		public static function post_content_navigation(){
			wp_link_pages( array(
				'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'bizsmart' ),
                'after'       => '</div>',
		...
		*/
		public static function single_post_navigation(){
			the_post_navigation( array(
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'bizsmart' ) . '</span><span class="nav-title">%title</span>',
				'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'bizsmart' ) . '</span><span class="nav-title">%title</span>',
			));
		}
		...
		public static function the_breadcrumb(){
			...
			if( apply_filters( self::fn_prefix( 'show_breadcrumb' ), true ) ): 			    
			    ...
			    <div id="<?php echo esc_attr( self::with_prefix( 'breadcrumb' ) ) ?>" class="<?php echo esc_attr( $classes ); ?>">
			    	<?php bizsmart_breadcrumb_trail( $breadcrumb_args ); ?>
				</div><!-- #breadcrumb -->
			    <?php return;
			endif;
		}
					...
					<?php 
						...
						<?php }elseif(  is_search() ){
							...
							<h2 class="entry-title">
								<?php 
									printf( 
										esc_html__( 'Search Results for: %s', 'bizsmart' ), 
