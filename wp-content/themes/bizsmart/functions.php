<?php
/**
 * BizSmart functions and definitions
 *
 * BizSmart only works in WordPress 4.7 or later.
 *
...
define( 'BIZSMART_PREFIX', 'bizsmart' );
define( 'BIZSMART_VERSION', '1.0.5' );

require get_parent_theme_file_path( '/classes/class-helper.php' );

final class BizSmart_Theme extends BizSmart_Helper{
	/**
	 * constructor
	 *
	 * @since 1.0.0
	...
	public function __construct(){
		...
		RT_Welcome_Notice::notice_init( 'BizSmart' );
		...
		# header main
		add_action( self::fn_prefix( 'header' ), array( __CLASS__, 'header' ), 20 );
		...
		add_action( self::fn_prefix( 'after_primary_menu' ), array( __CLASS__, 'header_button' ), 20 );		
		# adds search icon on header
		add_action( self::fn_prefix( 'after_primary_menu' ), array( __CLASS__, 'add_offcanvas_menu' ), 30 );
		...
		add_action( self::fn_prefix( 'after_footer' ), array( __CLASS__, 'offcanvas_menu_content' ) );
	}
	...
	public static function add_button(){
		$enable = bizsmart_get( 'enable-scroll-to-top' );
		if( !$enable ){
			return;
		}?>
	...
	*/
	public static function customize_register( $wp_customize ){
		...
		$wp_customize->get_control( 'header_textcolor' )->label = esc_html__( 'Site Identity Color', 'bizsmart' );

		# changing header image to Inner Banner options and adding inside theme option panel
		$wp_customize->get_section( 'header_image' )->title = esc_html__( 'Inner Banner Options', 'bizsmart' );
	...
	*/
	public static function meta_fields(){
		...
		$label = esc_html__( 'BizSmart Settings', 'bizsmart' );
		foreach ( $post_types as $type ) {

			$post = new BizSmart_Meta_Fields( $type );
			$options = array(
				'sidebar-position' => array(
					...
					'default' => 'customizer',
					'choices' => array(
						'customizer' 	=> esc_html__( 'From customizer', 'bizsmart' ),
						'left-sidebar' 	=> esc_html__( 'Left', 'bizsmart' ),
						'right-sidebar' => esc_html__( 'Right', 'bizsmart' ),
						'no-sidebar' 	=> esc_html__( 'Hide', 'bizsmart' ),
					),
				),
			);
			...
			if( 'page' == $type ){
				$page_options = array(
					'disable-inner-banner' => array(
						'type'  => 'checkbox',
						'label' => esc_html__( 'Disable Banner', 'bizsmart' ),
					),
					'disable-footer-widget' => array(
						'type'  => 'checkbox',
						'label' => esc_html__( 'Disable Footer Widget', 'bizsmart' ),
					),
				);
	...
	public static function top_bar(){
		if( !bizsmart_get( 'enable-top-bar' ) ){
			return;
		}

		if( '' == bizsmart_get( 'availability-text' ) && '' == bizsmart_get( 'contact-text' ) ){
			return;
		}
	...
	*/
	public static function get_excerpt_length() {
		return bizsmart_get( 'excerpt_length' );
	}
	...
	public static function get_blog_title(){
		...
		$ib_blog_title = bizsmart_get( 'ib-blog-title' );
	...
	public static function show_breadcrumb(){
		...
		return bizsmart_get( 'show-breadcrumb' );
	}
	...
	public static function show_preloader(){
		return bizsmart_get( 'pre-loader' );
	}
	...
	public static function show_post_author(){
		return bizsmart_get( 'post-author' );
	}
	...
	public static function show_post_category(){
		return bizsmart_get( 'post-category' );
	}
	...
	public static function supports(){
		 ...
		 */
		load_theme_textdomain( 'bizsmart', self::get_theme_path( 'languages' ) );
	}
	...
	public static function sidebars(){
		...
		register_sidebar( array(
	        'name' 			=> esc_html__( 'Sidebar', 'bizsmart' ),
	        'id' 			=> self::fn_prefix( 'sidebar' ),
	        'description' 	=> esc_html__( 'Widgets in this area will be shown on side of the page.', 'bizsmart' ),
		...
		#sliding menu
		register_sidebar( array(
	        'name' 			=> esc_html__( 'Off Canvas Area', 'bizsmart' ),
	        'id' 			=> self::fn_prefix( 'offcanvas_menu' ),
	        'description' 	=> esc_html__( 'Widgets in this area will be shown on sliding menu', 'bizsmart' ),
	        'before_widget' => '<section id="%1$s" class="widget %2$s">',
	        'after_widget'  => '</section>',
	        'before_title'  => '<h2 class="widget-title">',
	        'after_title'   => '</h2>',
		...
		$description = esc_html__( 'Widgets in this area will be displayed in the {position} column in the footer. If empty then column will not be displayed.', 'bizsmart' );
		for( $i = 1; $i <= bizsmart_get( 'layout-footer' ); $i++ ){
			switch ($i){
				case '1':
					$position = esc_html__( 'first', 'bizsmart' );
				break;
				case '2':
					$position = esc_html__( 'second', 'bizsmart' );
				break;
				case '3':
					$position = esc_html__( 'third', 'bizsmart' );
				break;
				case '4':
					$position = esc_html__( 'fourth', 'bizsmart' );
				break;
				default:
					$position = esc_html__( 'first', 'bizsmart' );
			...
			register_sidebar( array(
		        ...
		        'name' 			=> sprintf( esc_html__( 'Footer Widget Area %d ', 'bizsmart' ), $i ),
	...
	public static function nav_menu(){
		register_nav_menus(
			array(
				'primary' => esc_html__( 'Primary', 'bizsmart' ),
				'social-menu-footer' => esc_html__( 'Footer social menu', 'bizsmart' ),
				'footer-menu' => esc_html__( 'Footer Menu', 'bizsmart' )
			)
		);
	}
	...
	public static function get_sidebar_class( $classes ){
		...
		if( $page_template || is_search() || self::is_woo_single_page() || self::is_woo_product_category() ){
			...
			$customizer_position = bizsmart_get( 'sidebar-position' );
	...
	public static function inner_banner_class( $classes ){
		$classes[] = bizsmart_get( 'ib-text-align' );
		if( has_header_image() ){
			$classes[] = bizsmart_get( 'ib-image-attachment' );
		}
	...
	public static function register_required_plugins(){
		$plugins = array(
			array(
				'name'     => esc_html__( 'Rise Blocks - A Complete Gutenberg Page builder', 'bizsmart' ),
				'slug'     => 'rise-blocks',
				'required' => false,
			)
		);
	...
	public static function header_button(){
		if( '' != bizsmart_get( 'header-btn-text' ) ){ ?>
			<div class="bizsmart-header-button">
				<a href="<?php echo esc_attr( bizsmart_get( 'header-button-url' ) ); ?>"> <?php echo bizsmart_get( 'header-btn-text' ); ?> </a>
			</div>
		<?php }
	}
	...
	public static function add_offcanvas_menu(){ 
		if( !bizsmart_get( 'enable-offcanvas' ) ){
			return;
		}?>			
		<div class="bizsmart-offcanvas-menu" id="offcanvas-menu-icon">
			<span class="text"><?php esc_html_e( 'Slide Area', 'bizsmart' ); ?></span>
	...
	*/
	public static function footer_social_menu(){
		if( !bizsmart_get( 'footer-social-menu' ) ){
			return;
		}
	...
	*/
	public static function the_default_search(){ ?>
        <section class="widget widget_search">
			<h2 class="widget-title"><?php esc_html_e( 'Search', 'bizsmart' ); ?></h2>	        	
           <?php get_search_form(); ?>
        </section>
	<?php }
	...
	*/
	public static function the_default_recent_post(){ ?>
        <section class="widget widget_recent_entries">
			<h2 class="widget-title"><?php esc_html_e( 'Recent Posts', 'bizsmart' ); ?></h2>
			<?php $pst = BizSmart_Theme::get_posts_by_type( 'latest', 0, 5 ); 
			if( !empty( $pst ) ):?>
				<ul>
	...
	*/
	public static function the_default_archive(){ ?>
		<section class="widget">
		    <h3 class="widget-title"><?php esc_html_e( 'Archives', 'bizsmart' ); ?></h3>
		    <ul>
	...
	*/
	...
	public static function the_post_per_row_class( $class = '' ){
		$post_per_row = bizsmart_get( 'post-per-row' );
		if( '1' == $post_per_row ){
			$class = 'col-md-12';
	 ...
	 * @package BizSmart WordPress Theme
	 */
	public static function offcanvas_menu_content(){
		if ( is_active_sidebar( 'bizsmart_offcanvas_menu' ) ) { ?>
			<div class="bizsmart-offcanvas-overlay"></div>
			<div class="bizsmart-offcanvas-menu-content">
				<a href="#" class="offcanvas-close-icon"></a>
				<?php dynamic_sidebar( 'bizsmart_offcanvas_menu' ); ?>
			</div>			
		<?php }
	}
}
